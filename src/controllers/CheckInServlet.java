    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {  	
        ...
        System.out.println("GQLoginController user my id = " + user.getMyID());
			...
			if(uh.alreadyBanned(user.getUserRecordID())) {
				...
				System.out.println("Check In already banned! " + user.getMyID() + " " + user.getUserFirstName() + " " + user.getUserLastName());				
				...
				if (inTable){ // if in the user table update the last login filed
					...
					System.out.println("Check In in table " + user.getUserFirstName() + user.getUserLastName());	
					...
					System.out.println("Check In NOT in user table recd id" + user.getMyID() + " " + user.getUserFirstName() + " " + user.getUserLastName() + " " + user.getUserEmail());	
				}
				...
				System.out.println("Check In get user Record ID =  " + userRecordID);			
				...
				if (buildingID == 0){
					...
					System.out.println("Check In building qr name doesn't exist " + building);
					...
					System.out.println("Check In room qr # " + room + " has recd id = " + roomID);

					// check if room id = 0, and if so send to error page w/ message
					
					if (roomID == 0){ //have an error						
						
						System.out.println("Check In room qr name doesn't exist " + room);
						...
						if (reservationID == 0){ // didn't find reservation 
							...
							System.out.println("Check In reservation not found ");
							...
							System.out.println("Check In reservation found - checking in ");
